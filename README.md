...
**확인된 사실**: 같은 계정으로 공식 코레일+ 앱은 정상 동작합니다. 즉 차단은 계정이 아니라
**클라이언트를 구분해서** 걸려 있습니다. 코레일이 요청을 보고 진짜 앱인지 아닌지 판별합니다.

이 도구는 그 차단을 **우회하지 않습니다.** 차단 응답을 받으면 재시도 없이 즉시 멈추고,
그 사실을 `data/blocked.json` 에 기록해 **24시간 동안은 요청을 보내기 전에 스스로 거절**합니다.
계속 두드려봐야 결과는 같고 실패 로그인만 쌓이는데, 그게 쌓이면 지금은 멀쩡한 계정까지
플래그가 붙을 수 있기 때문입니다.

```bash
python -m railcatch unblock          # 정책이 바뀐 것 같을 때 기록 삭제
python -m railcatch doctor --force   # 기록을 무시하고 한 번 시도 (권장하지 않음)
```
...
브라우저에서 `http://127.0.0.1:8777` 이 열립니다. 예약대기 관리 화면입니다.

- **출발역 / 도착역** — 목록에서 선택 (⇄ 버튼으로 방향 뒤집기)
- **날짜 / 출발 시각** — 달력·시계 입력
- **열차 종류** — KTX, KTX-산천, ITX-새마을 등 목록에서 선택. 번호는 선택 입력
- **좌석** — 일반실 / 특실 / 상관없음

추가한 뒤 코레일+ 앱에서 실제로 예약대기를 걸고, 목록에서 **«대기 걸었음»** 을 누르면
됩니다. 배정되면 **«배정됨»** 을 누르고 결제 기한을 넣어두세요 — 기한까지 2시간마다
알려줍니다.

웹 UI를 켜두면 알림 확인도 백그라운드에서 같이 돕니다. 별도로 `waitlist watch` 를
띄울 필요가 없습니다.
...
```bash
# 예약대기 걸 후보 열차를 등록 (웹 UI에서 드롭다운으로 해도 됩니다)
python -m railcatch waitlist add "KTX 101" "서울→부산" 9/20 08:30 --seat general
